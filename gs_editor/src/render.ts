...
import { DLSSSettings } from './ui/dlss-settings-dialog';
...
const registerRenderEvents = (scene: Scene, events: Events) => {
    ...
    // DLSS Settings event handler
    events.function('dlss.configure', (dlssSettings: DLSSSettings) => {
        if (scene.simpleDLSS) {
            console.log('🎯 DLSS: Configuring simple DLSS...', dlssSettings);
            
            // 先设置质量模式，再启用
            scene.simpleDLSS.setQualityMode(dlssSettings.qualityMode);
            scene.simpleDLSS.setEnabled(dlssSettings.enabled);
            
            // Force a re-render to apply the changes
            scene.forceRender = true;
            
            console.log('✨ DLSS: Configuration completed');
            
        } else {
            console.error('❌ DLSS: Simple DLSS not initialized!');
        }
    });

    // Get DLSS performance metrics
    events.function('dlss.getMetrics', () => {
        return scene.simpleDLSS ? scene.simpleDLSS.getPerformanceMetrics() : null;
    });

    // Check if DLSS is supported
    events.function('dlss.isSupported', () => {
        try {
            // Simple DLSS is always supported in modern browsers
            return scene.simpleDLSS !== null;
        } catch (e) {
            return false;
        }
    });
...
export { ImageSettings, VideoSettings, DLSSSettings, registerRenderEvents };
