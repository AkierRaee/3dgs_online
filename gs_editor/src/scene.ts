...
import { HotspotAdjuster } from './hotspot-adjuster';
...
import { SimpleDLSS } from './simple-dlss';
...
class Scene {
    ...
    hotspotAdjuster: HotspotAdjuster;
    simpleDLSS: SimpleDLSS;
    ...
    constructor(
        ...
        // 初始化热点渲染器
        this.hotspotRenderer = new HotspotRenderer(events, this.app);
        
        // 初始化热点调整器
        this.hotspotAdjuster = new HotspotAdjuster(events);
        
        // 初始化简化版DLSS
        this.simpleDLSS = new SimpleDLSS(this.app.graphicsDevice, events);
    }
    ...
    private onPreRender() {
        if (this.canvasResize) {
            ...
            // 保存当前DLSS设置
            const currentSettings = this.simpleDLSS ? {
                enabled: this.simpleDLSS.getPerformanceMetrics().enabled,
                qualityMode: this.simpleDLSS.getPerformanceMetrics().qualityMode
            } : { enabled: false, qualityMode: 'quality' as const };
            
            // Update SimpleDLSS when canvas size changes
            this.simpleDLSS.destroy();
            this.simpleDLSS = new SimpleDLSS(this.app.graphicsDevice, this.events);
            
            // 恢复DLSS设置
            if (currentSettings.enabled) {
                this.simpleDLSS.setQualityMode(currentSettings.qualityMode);
                this.simpleDLSS.setEnabled(true);
            }
        }
    ...
    private onPostRender() {
        ...
        // Update DLSS performance metrics
        this.updateDLSSMetrics();
    ...
    updateDLSSMetrics() {
        if (this.simpleDLSS) {
            this.simpleDLSS.updatePerformanceMetrics();
        }
    }
}
