...
class PointerController {
    ...
    constructor(camera: Camera, target: HTMLElement) {
        ...
        this.update = (deltaTime: number) => {
            ...
            const wasdZ = keys.s - keys.w; // W前进，S后退
            const wasdY = keys.e - keys.q; // Q下降，E上升

            // 合并箭头键和WASD输入
            const x = arrowx + wasdX;
            const z = arrowz + wasdZ;
            const y = wasdY; // 垂直移动仅由WASDQE控制

            if (x || z || y) {
                const factor = deltaTime * camera.flySpeed;
                camera.moveByInput(x, y, z, factor);
            }
        };
