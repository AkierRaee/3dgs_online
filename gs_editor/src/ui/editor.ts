...
import { DLSSSettingsDialog } from './dlss-settings-dialog';
...
class EditorUI {
    ...
    constructor(events: Events, remoteStorageMode: boolean) {
        ...
        // FPS counter
        const fpsCounter = new FPSCounter(events);
        canvasContainer.append(fpsCounter);
        ...
        const dlssSettingsDialog = new DLSSSettingsDialog(events);
        ...
        topContainer.append(dlssSettingsDialog);
        ...
        events.function('show.dlssSettingsDialog', async () => {
            const dlssSettings = await dlssSettingsDialog.show();

            if (dlssSettings) {
                await events.invoke('dlss.configure', dlssSettings);
            }
        });
