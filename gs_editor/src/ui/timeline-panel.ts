...
class TimelinePanel extends Container {
    constructor(events: Events, tooltips: Tooltips, args = {}) {
        ...
        buttonControls.append(saveKeys);
        ...
        tooltips.register(saveKeys, '保存关键帧到文件', 'top');
        tooltips.register(loadKeys, '从文件加载关键帧', 'top');
        tooltips.register(uploadImages, '上传图片文件夹', 'top');
        tooltips.register(jumpNearest, '跳转到最近关键帧并对齐视角', 'top');
        tooltips.register(simplifyToggle, '简化关键帧（仅取10个）', 'top');
