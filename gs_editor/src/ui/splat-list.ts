    ...
    constructor(events: Events, args = {}) {
        ...
        events.on('scene.elementAdded', (element: Element) => {
            if (element.type === ElementType.splat) {
                ...
                const shouldBeVisible = isFirstSplat;
                splat.visible = shouldBeVisible;
                item.visible = shouldBeVisible;
        ...
        events.on('scene.elementRemoved', (element: Element) => {
            if (element.type === ElementType.splat) {
                ...
                if (item) {
                    ...
                    if (events.invoke('selection') === splat && items.size > 0) {
                        const firstSplat = items.keys().next().value;
                        if (firstSplat) {
                            showOnlySplat(firstSplat);
            ...
            for (const [key, value] of items) {
                if (item === value) {
                    ...
                    setTimeout(() => {
                        ...
                        console.log(`切换到PLY "${key.name}" 并自动开始播放`);
                    }, 50);
