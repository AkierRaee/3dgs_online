...
class Camera extends Element {
    ...
    // unified movement for keyboard input (x: right, y: world up, z: forward)
    moveByInput(x: number, y: number, z: number, factor: number) {
        // any manual input switches to orbit mode
        this.exitFreeMode();

        const worldTransform = this.entity.getWorldTransform();
        const dx = worldTransform.getX().mulScalar(x * factor);
        const dz = worldTransform.getZ().mulScalar(z * factor);
        const dy = new Vec3(0, y * factor, 0);
        const delta = dx.add(dz).add(dy);

        const p = this.focalPoint.add(delta);
        this.setFocalPoint(p);
    }
    ...
    rotateFree(dx: number, dy: number) {
        ...
        // world-up yaw
        const qYaw = new Quat().setFromEulerAngles(0, yawDeg, 0);
