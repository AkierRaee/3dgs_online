...
const registerEditorEvents = (events: Events, editHistory: EditHistory, scene: Scene) => {
    ...
    const performSelectionFunc = async (func: 'duplicate' | 'separate') => {
        ...
        if (buffer) {
            ...
            // wrap PLY in a blob and load it (ensure ArrayBuffer type for TS/dom)
    ...
    events.function('camera.position', () => {
        return scene.camera.entity.getPosition();
    });

    events.function('camera.entity', () => {
        return scene.camera.entity;
    });
    ...
    // bridge: camera.setView
    // Usage examples:
    // events.fire('camera.setView', { position, rotation, fx, fy, speed });
    // events.fire('camera.setView', { position, target, rotation, fx, fy, speed });
    events.on('camera.setView', (payload: any) => {
        ...
        const speed: number | undefined = payload.speed;
